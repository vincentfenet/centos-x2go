...
RUN yum -y install wget sudo openssh-server git xinetd socat make dos2unix httpd nano elinks pytz glibc.i686 zip unzip cpanminus which file mailx mutt bind-utils vim python2 python-pip python-virtualenv dnf samba samba-client expect libxml2-devel libxslt-devel poppler-utils && yum -y group install "Development Tools" && yum clean all
